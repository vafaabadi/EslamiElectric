runWhenLocaleReady(function () {
    ...
    function productPrimaryImageAlt(p) {
      if (currentLang === 'fa') {
        if (p.image_alt_fa) return String(p.image_alt_fa);
        if (p.image_alt_en) return String(p.image_alt_en);
      } else {
        if (p.image_alt_en) return String(p.image_alt_en);
        if (p.image_alt_fa) return String(p.image_alt_fa);
      }
    ...
    function productDescriptionSnippetForLocale(p) {
      const raw = currentLang === 'fa' && p.description_fa ? p.description_fa : (p.description || '');
    ...
    function renderProducts(categoryId) {
      ...
      productsToShow.forEach((p, idx) => {
        ...
        const imgAlt = productPrimaryImageAlt(p);
        const descSnippet = productDescriptionSnippetForLocale(p);
