'use strict';

const { z } = require('zod');

/** POST /api/notify/signup — activation email + Telegram notify. */
const notifySignupBodySchema = z
  .object({
    type: z.enum(['person', 'company']).optional(),
    skipEmail: z.boolean().optional(),
    firstName: z.string().max(100).optional(),
    surname: z.string().max(100).optional(),
    email: z.string().min(1).max(320),
    dob: z.union([z.string(), z.null()]).optional(),
    mobile: z.string().max(40).optional(),
    landline: z.union([z.string(), z.null()]).optional(),
    address: z.union([z.string(), z.null()]).optional(),
    bankDetails: z.union([z.string(), z.null()]).optional(),
    companyName: z.union([z.string(), z.null()]).optional(),
    companyNumber: z.union([z.string(), z.null()]).optional(),
    companyContactNumber: z.union([z.string(), z.null()]).optional(),
    companyPrincipalContact: z.union([z.string(), z.null()]).optional()
  })
  .strict();

/** POST /api/account/request-deletion */
const accountDeletionBodySchema = z
  .object({
    confirmPhrase: z.string().max(120),
    password: z.string().max(2048).optional()
  })
  .strict();

/** PATCH /api/me — only known profile keys; no account email change here. */
const profilePatchBodySchema = z
  .object({
    firstName: z.string().optional(),
    surname: z.string().optional(),
    dob: z.union([z.string(), z.null()]).optional(),
    mobile: z.string().optional(),
    landline: z.union([z.string(), z.null()]).optional(),
    contactEmail: z.union([z.string(), z.null()]).optional(),
    address: z.string().optional(),
    bankDetails: z.union([z.string(), z.null()]).optional(),
    companyName: z.string().optional(),
    companyNumber: z.string().optional(),
    companyContactNumber: z.union([z.string(), z.null()]).optional(),
    companyPrincipalContact: z.union([z.string(), z.null()]).optional()
  })
  .strict();

/** POST /api/claim-account */
const claimAccountBodySchema = z
  .object({
    token: z.string().min(1).max(256),
    password: z.string().min(1).max(2048),
    confirmPassword: z.string().max(2048).optional()
  })
  .strict();

const lineItemInputSchema = z
  .object({
    price: z.union([z.number(), z.string()]).optional(),
    name: z.string().optional(),
    quantity: z.union([z.number(), z.string()]).optional(),
    id: z.union([z.string(), z.number()]).optional(),
    productId: z.union([z.string(), z.number()]).optional()
  })
  .passthrough();

/** POST /api/create-checkout-session */
const createCheckoutSessionBodySchema = z
  .object({
    priceId: z.string().max(200).optional(),
    amount: z.union([z.number(), z.string()]).optional(),
    lineItems: z.array(lineItemInputSchema).max(200).optional(),
    guestEmail: z.string().max(320).optional(),
    guestName: z.string().max(200).optional(),
    guestPhone: z.string().max(40).optional(),
    shippingAddress: z.record(z.string(), z.unknown()).optional(),
    locale: z.enum(['en', 'fa']).optional(),
    fulfillmentType: z.enum(['collection', 'delivery']).optional(),
    pendingOrderId: z.string().max(200).optional(),
    paymentMethod: z.enum(['stripe', 'crypto']).optional(),
    payCurrency: z.string().max(32).optional()
  })
  .strict();

/** POST /api/orders/guest-cancel, guest-resume-checkout */
const guestOrderTokenBodySchema = z
  .object({
    token: z.string().min(10).max(512),
    locale: z.enum(['en', 'fa']).optional()
  })
  .strict();

/** POST /api/orders/:orderId/resume-checkout */
const resumeCheckoutBodySchema = z
  .object({
    locale: z.enum(['en', 'fa']).optional()
  })
  .strict();

/** POST /api/orders/confirm-by-session/:sessionId — no fields expected. */
const emptyJsonBodySchema = z.object({}).strict();

/** GET /api/orders/guest-lookup */
const guestLookupQuerySchema = z
  .object({
    email: z.string().min(1).max(320),
    order_id: z.string().min(1).max(120)
  })
  .strict();

/** DELETE /api/admin/products/:productId — no JSON body. */

/** DELETE /api/admin/categories/:categoryId — no JSON body. */

/** PATCH /api/admin/products/:productId */
const adminProductPatchBodySchema = z
  .object({
    name: z.string().min(1).max(220).optional(),
    name_fa: z.string().max(220).optional(),
    price: z.union([z.number(), z.string()]).optional(),
    description: z.string().max(4000).optional(),
    description_fa: z.string().max(4000).optional(),
    image_url: z.string().max(4000).optional(),
    image_alt_en: z.union([z.string().max(500), z.null()]).optional(),
    image_alt_fa: z.union([z.string().max(500), z.null()]).optional(),
    category_id: z.string().min(1).max(120).optional(),
    wattage: z.union([z.number(), z.null()]).optional()
  })
  .strict();

const slugSafeIdSchema = z.string().regex(/^[a-z0-9-]+$/, 'id must match ^[a-z0-9-]+$').max(120);

/** POST /api/admin/categories — `id` optional for power users; server otherwise slugifies English `name`. */
const adminCategoryPostBodySchema = z
  .object({
    name: z.string().min(1).max(220),
    name_fa: z.string().max(220).optional(),
    sort_order: z.number().int().optional(),
    id: slugSafeIdSchema.optional()
  })
  .strict();

/** PATCH /api/admin/categories/:categoryId */
const adminCategoryPatchBodySchema = z
  .object({
    name: z.string().min(1).max(220).optional(),
    name_fa: z.string().max(220).optional(),
    sort_order: z.number().int().optional()
  })
  .strict()
  .refine((o) => o.name != null || o.name_fa != null || o.sort_order != null, {
    message: 'At least one of name, name_fa, sort_order is required'
  });

/** PATCH /api/admin/categories/reorder */
const adminCategoriesReorderBodySchema = z
  .object({
    orderedIds: z.array(slugSafeIdSchema).min(1)
  })
  .strict();

/** POST /api/admin/products */
const adminProductPostBodySchema = z
  .object({
    category_id: z.string().min(1).max(120),
    name: z.string().min(1).max(220),
    name_fa: z.string().max(220).optional(),
    price: z.union([z.number(), z.string()]),
    description: z.string().max(4000).optional(),
    description_fa: z.string().max(4000).optional(),
    image_url: z.string().max(4000).optional(),
    image_alt_en: z.string().max(500).optional(),
    image_alt_fa: z.string().max(500).optional(),
    extra_json: z.record(z.string(), z.unknown()).optional(),
    wattage: z.union([z.number(), z.null()]).optional()
  })
  .strict();

/** POST /api/admin/products/:id/duplicate */
const adminProductDuplicateBodySchema = z
  .object({
    suffix_name: z.boolean().optional()
  })
  .strict();

/** POST /api/me/push-tokens — register/upsert an FCM token for the current user. */
const pushTokenPostBodySchema = z
  .object({
    token: z.string().min(20).max(4096),
    platform: z.enum(['android', 'ios', 'web']).optional(),
    appVersion: z.string().max(60).optional(),
    locale: z.enum(['en', 'fa']).optional()
  })
  .strict();

/** DELETE /api/me/push-tokens — body: token to remove (logout). */
const pushTokenDeleteBodySchema = z
  .object({
    token: z.string().min(20).max(4096)
  })
  .strict();

/** PATCH /api/me/push-preferences */
const pushPreferencesPatchBodySchema = z
  .object({
    master_enabled: z.boolean().optional(),
    channels: z
      .object({
        orders: z.boolean().optional(),
        promotions: z.boolean().optional(),
        account: z.boolean().optional(),
        general: z.boolean().optional()
      })
      .strict()
      .optional()
  })
  .strict()
  .refine((o) => o.master_enabled != null || (o.channels && Object.keys(o.channels).length > 0), {
    message: 'At least one of master_enabled or channels is required'
  });

/** PUT /api/me/basket-activity and PUT /api/basket-activity */
const basketActivityItemSchema = z
  .object({
    id: z.string().max(200).optional(),
    categoryId: z.string().max(200).optional().nullable(),
    category_id: z.string().max(200).optional().nullable(),
    name: z.string().min(1).max(500),
    name_fa: z.string().max(500).optional().nullable(),
    nameFa: z.string().max(500).optional().nullable(),
    image_url: z.string().max(2000).optional().nullable(),
    imageUrl: z.string().max(2000).optional().nullable(),
    price: z.number().nonnegative().max(1e9),
    quantity: z.number().int().positive().max(9999)
  })
  .strict();

const basketActivityPutBodySchema = z
  .object({
    items: z.array(basketActivityItemSchema).max(200)
  })
  .strict();

/** POST /api/admin/push/broadcast — promotional FCM broadcast */
const adminPushBroadcastBodySchema = z
  .object({
    title_en: z.string().min(1).max(120),
    title_fa: z.string().min(1).max(120),
    body_en: z.string().min(1).max(500),
    body_fa: z.string().min(1).max(500),
    channel: z.enum(['promotions']).optional()
  })
  .strict();

const chatMessageSchema = z
  .object({
    role: z.enum(['user', 'assistant']),
    content: z.string().min(1).max(4000)
  })
  .strict();

/** POST /api/chat — AI assistant */
const chatBodySchema = z
  .object({
    messages: z.array(chatMessageSchema).min(1).max(30),
    locale: z.enum(['en', 'fa']).optional(),
    sessionId: z.string().max(64).optional()
  })
  .strict();

/** PATCH /api/admin/orders/:orderId — staff fulfillment */
const adminOrderPatchBodySchema = z
  .object({
    fulfillment_status: z
      .enum(['unfulfilled', 'processing', 'shipped', 'delivered', 'cancelled'])
      .optional(),
    tracking_number: z.union([z.string().max(200), z.null()]).optional(),
    admin_notes: z.string().max(8000).optional()
  })
  .strict()
  .refine((o) => o.fulfillment_status != null || o.tracking_number !== undefined || o.admin_notes !== undefined, {
    message: 'At least one of fulfillment_status, tracking_number, admin_notes is required'
  });

module.exports = {
  notifySignupBodySchema,
  accountDeletionBodySchema,
  profilePatchBodySchema,
  claimAccountBodySchema,
  createCheckoutSessionBodySchema,
  guestOrderTokenBodySchema,
  resumeCheckoutBodySchema,
  emptyJsonBodySchema,
  guestLookupQuerySchema,
  adminProductPatchBodySchema,
  adminCategoryPostBodySchema,
  adminCategoryPatchBodySchema,
  adminCategoriesReorderBodySchema,
  adminProductPostBodySchema,
  adminProductDuplicateBodySchema,
  adminOrderPatchBodySchema,
  pushTokenPostBodySchema,
  pushTokenDeleteBodySchema,
  pushPreferencesPatchBodySchema,
  basketActivityPutBodySchema,
  adminPushBroadcastBodySchema,
  chatBodySchema
};
