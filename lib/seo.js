...
/** Routes that should not be indexed (auth-only, recovery, transactional confirmation). Key shop pages stay indexable. */
...
const SEO_ROUTES = {
  ...
  privacy: {
    en: {
      title: 'Privacy Policy | Eslami Electric',
      description:
        'How Eslami Electric collects, uses, and protects your data on our website and Android app — orders, payments via Stripe, account info, and your rights.'
    },
    fa: {
      title: 'سیاست حریم خصوصی | الکتریکی اسلامی',
      description:
        'نحوه جمع‌آوری و استفاده از داده‌های شما در وب‌سایت و اپ اندروید الکتریکی اسلامی — سفارش، پرداخت Stripe، حساب کاربری و حقوق شما.'
    }
  }
};
...
function injectSeoBundle(html, { baseUrl, locale, routeKey, categories: categoriesRaw, requestPathAndQuery }) {
  ...
  const canonicalHomeUrl = absoluteUrl(baseUrl, buildCanonicalPath(loc, ''));

  /** One stable @id for LocalBusiness across locales (matches English home URL anchor). */
